...
/// A user-installed plugin: one folder under the plugins directory holding a
/// `manifest.json`, an executable, and (optionally) an `icon.png`.
///
/// The engine is deliberately tiny — a plugin is a *one-shot action*: we feed it
...
/// What feeds the plugin's stdin.
enum PluginInput: String, Codable {
...
/// Where the plugin's stdout goes.
enum PluginOutput: String, Codable {
...
/// sandbox the process, so this tells the user what a plugin *claims* it needs,
/// not what it's prevented from doing. (Enforcement via `sandbox-exec` is a
/// later layer.) The keychain vault is never handed to a plugin regardless.
enum PluginPermission: String, Codable, CaseIterable {
    case network
    case files
}

/// How the plugin is triggered.
...
struct PluginManifest: Codable, Equatable {
    ...
    let mode: PluginMode
    let keybind: String?         // single character; runs the plugin while the tab is focused
    ...
    init(from decoder: Decoder) throws {
        ...
        input = try c.decodeIfPresent(PluginInput.self, forKey: .input) ?? .none
        output = try c.decodeIfPresent(PluginOutput.self, forKey: .output) ?? .none
        permissions = try c.decodeIfPresent([PluginPermission].self, forKey: .permissions) ?? []
        mode = try c.decodeIfPresent(PluginMode.self, forKey: .mode) ?? .instant
...
/// Editable form-state for creating or editing a plugin in-app. Kept separate
/// from `PluginManifest` (read model) so the editor can hold half-finished
/// values; `manifestDictionary()` serialises it back out cleanly.
struct PluginDraft {
    var name: String = ""
    var symbol: String = "wand.and.stars"
    var colorHex: String = PluginPalette.swatches[0]
    var details: String = ""
    var input: PluginInput = .clipboard
    var output: PluginOutput = .copy
    var mode: PluginMode = .instant
    var network: Bool = false
    var files: Bool = false
    var keybind: String = ""
    var script: String = "#!/bin/bash\n# stdin = your input · stdout = the result\ncat\n"

    /// Folder name this draft edits, or nil for a brand-new plugin.
    var existingFolder: String?

    init() {}

    init(from plugin: Plugin, script: String) {
        existingFolder = plugin.id
        let m = plugin.manifest
        name = m.name
        symbol = m.icon ?? "wand.and.stars"
        colorHex = m.color ?? PluginPalette.swatches[0]
...
/// for seeded plugins). Hex strings so they round-trip through the manifest.
enum PluginPalette {
...
struct PluginRunResult: Identifiable {
    let id = UUID()
